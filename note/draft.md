...
# Chapter 7
## 奈奎斯特稳定性判据（判断`任意G(s)和H(s)`组成的`闭环系统稳定性`）
...
## 奈奎斯特稳定性判据应用：开环传输函数极坐标图与闭环系统稳定性(`G(s)和H(s)均稳定`情况下`闭环系统稳定性`判断的子结论)
...
- 由于$G(s)H(s)$平面上的包围圈由$G(j\omega)H(j\omega)$轨迹组成，其中$\omega$从$(-\infty,0)$和$(0,\infty)$的轨迹关于横坐标轴对称，因此只要$(0,\infty)$的轨迹对(-1,0)有半包围趋势，那么完整的轨迹一定对(-1,0)包围，即函数$A(s)$所对应的系统不稳定；如果$(0,\infty)$的轨迹对(-1,0)无半包围趋势，那么完整的轨迹一定不对(-1,0)包围，即函数$A(s)$所对应的系统稳定。由于$(0,\infty)$的轨迹即是$G(j\omega)H(j\omega)$的极坐标图，因此$G(j\omega)H(j\omega)$的极坐标图对(-1,0)有无半包围趋势决定了函数$A(s)$所对应的系统是否稳定。
- 对于绝大多数实际工程情况，只要$G(j\omega)H(j\omega)$的极坐标图与横坐标轴的交点小于-1,那么$G(j\omega)H(j\omega)$的极坐标图对(-1,0)就不存在半包围趋势，函数$A(s)$所对应的系统就稳定。因此通过调整$G(s)H(s)$中的变量值，实现G(j\omega)H(j\omega)$的极坐标图与横坐标轴的交点小于-1，从而实现函数$A(s)$所对应的系统就稳定。
## 前馈传输函数$G(s)$与闭环系统稳定性的裕度要求
- 其中$H(s)$为1时的$G(s)H(s)$是一种特殊情况，此时$G(s)H(s)$等价于$G(s)$，即$G(s)$的极坐标图与$G(s)$接成单位增益负反馈结构时开环增益$G(s)H(s)=G(s)*1=G(s)$的极坐标图一致，从$G(s)$的极坐标图图中可以看出单位增益负反馈下系统是否稳定，可以看出何种形式的$H(s)$能够实现$G(s)H(s)$的极坐标图不包围(-1,0)。
- 因此$G(s)$的极坐标图中增益为1时的相位差相较于$-180$度的差值反映了对$H(s)$的相位差要求，称为相位裕度。
- $G(s)$的极坐标图中相位差为1时的增益相较于$1$的比值反映了对$H(s)$的增益要求，称为增益裕度。
...
- 注：对于F(s)=“s的表达式”，`自变量s的取值`映射了`在该自变量s取值下F(s)的取值`，将自变量s的实部和虚部分别作为点的横纵坐标标注于坐标系平面中，那么这个坐标系平面中的点的横纵坐标也就映射了自变量s的取值，因此称这个平面为`s平面`；同理F(s)的实部和虚部分别作为点的横纵坐标标注于坐标系平面中，那么这个坐标系平面中的点的横纵坐标也就映射了F(s)的值，因此称这个平面为`F(s)平面`。映射关系是可以自由定义的，但是所有的映射只在该定义下才成立。
